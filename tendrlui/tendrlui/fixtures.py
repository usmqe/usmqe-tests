...
Just add the following line into your `conftest.py`::
    ...
    pytest_plugins = ["tendrlui.fixtures"]

"""
import pytest

from tendrlui.session import Session


@pytest.fixture()
def session(request):
    """Session fixture which automatically initializes (but does not start!)
    tendrlui UI session and correctly passes current test name to it.
...
def autosession(request):
    """Session fixture which automatically initializes and starts tendrlui UI
