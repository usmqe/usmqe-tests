...
try:
    ...
    # Optional requirement, tendrlui will report results back to saucelabs if
    # installed
    sauceclient = None
...
class SeleniumBrowserFactory(object):
    ...
    def get_browser(self):
        ...
        """
        if self.provider == 'selenium' or self.provider is None:
    ...
    def _get_selenium_browser(self):
        ...
        elif self.browser == 'firefox' or self.browser is None:
            if binary:
                kwargs.update({'executable_path': binary})
...
class DockerBrowser(object):
    ...
    def __init__(self, name=None, image=None, capabilities=None):
        ...
        :raises: tendrlui.browser.DockerBrowserError: if ``docker-py`` package is
            not installed.
        """
        if docker is None:
...
class TendrluiBrowserPlugin(DefaultPlugin):
    """Plug-in for :class:`TendrluiBrowser` which adds satellite-specific
...
class TendrluiBrowser(Browser):
    """A wrapper around :class:`widgetastic.browser.Browser` which injects
    :class:`tendrlui.session.Session` and :class:`TendrluiBrowserPlugin`.
    """
    ...
    def __init__(self, selenium, endpoint=None, session=None,
        ...
        :param selenium: :class:`selenium.webdriver.remote.webdriver.WebDriver`
            instance.
        :param session: :class:`tendrlui.session.Session` instance.
        ...
        super(TendrluiBrowser, self).__init__(
            selenium,
            plugin_class=TendrluiBrowserPlugin,
