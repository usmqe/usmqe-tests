...
from tendrlui.base.browser_manager import BrowserManager
from tendrlui.base.application.implementations import TendrluiImplementationContext, Implementation
from tendrlui.views.common import BaseLoggedInView, BasePage
...
class TendrluiNavigateStep(NavigateStep):
    VIEW = None
    ...
    def go(self, _tries=3, *args, **kwargs):
        ...
        :return: view instance if class attribute ``VIEW`` is set or ``None``
            otherwise
        """
        super(TendrluiNavigateStep, self).go(_tries=_tries, *args, **kwargs)
...
class WebUI(Implementation):
    ...
    def widgetastic_browser(self):
        ...
        return TendrluiBrowser(selenium_browser, self)
...
class LoginScreen(TendrluiNavigateStep):
    VIEW = BasePage
...
@WebUI.register_destination_for(WebUI)
class LoggedIn(TendrluiNavigateStep):
